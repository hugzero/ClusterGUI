...
public class GeoHash {
	...
	public static void main(String[] args) {
		...
		File file = new File("C:/Users/10960/Desktop/lantitute.txt");
		// ReadTxt.txt2String(file);
		try {
			BufferedReader br = new BufferedReader(new FileReader(file));// 构造一个BufferedReader类来读取文件
			String s = "";
			while ((s = br.readLine()) != null) {// 使用readLine方法，一次读一行
				String[] splitll = s.split(",");
				listGeo.add(new GeoHash(Double.parseDouble(splitll[0]), Double.parseDouble(splitll[1])));
				//
				// listGeo.add(new GeoHash(lat, lng));
			}
			br.close();
		} catch (Exception e) {
			e.printStackTrace();
		...
		for (int i = 0; i < listGeo.size(); i++) {
			System.out.println(listGeo.get(i).getGeoHashBase32());
		}
		// GeoHash g = new GeoHash(32.65093803405762, 120.20605087280273);
		// String geoHash = g.getGeoHashBase32();
		// System.out.println(geoHash);
		// LocationBean bean = g.getLocation(geoHash);
		// System.out.println(JsonUtil.parseJson(bean));
		// System.out.println(new GeoHash(bean.getLat(),
		// bean.getLng()).getGeoHashBase32());
		// System.out.println(DistanceUtil.getDistance(bean.getLat(),
		// bean.getLng(), bean.getLat() - g.minLat, bean.getLng() - g.minLng));
	}
}
